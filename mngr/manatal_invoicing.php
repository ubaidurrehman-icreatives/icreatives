...
function Contact_Info($link, $organization) {
    $f_query = "select full_name, full_name_on_invoice, terms, email, phone_number, one_invoice_per_candidate,address1,address2,city,state,postalcode,country,created_at from ic_company where organization = '" . $organization . "'";
...
<form method="post">
Week Ending:  <input type="date" id="week_ending_start" name="week_ending_start" > 
Week Ending:  <input type="date" id="week_ending_end" name="week_ending_end" > 
...
$strSQL = $strSQL . "SELECT  'TIMEACT' as '!TIMEACT' ";
...
$strSQL = $strSQL . ", NULLIF(TRIM(oj.po_number), '') AS PO ";
$strSQL = $strSQL . ", oj.po_amount as POAMT ";
...
$strSQL = $strSQL . ", c.terms as TERMS ";
$strSQL = $strSQL . ", c.email as EMAIL ";
$strSQL = $strSQL . ", c.address1 as ADDR1 ";
$strSQL = $strSQL . ", c.address2 as ADDR2 ";
$strSQL = $strSQL . ", c.city as CITY ";
$strSQL = $strSQL . ", c.state as STATE ";
$strSQL = $strSQL . ", c.postalcode as ZIP ";
$strSQL = $strSQL . ", c.billing_cycle as CYCLE ";
...
$strSQL = $strSQL . "RIGHT JOIN ic_matches oj ON (oj.candidate = wt.employee_id AND oj.job = wt.AssignmentNumber) ";
$strSQL = $strSQL . "INNER JOIN ic_company c ON (c.organization = oj.organization and c.deactivated <> 1) ";
$strSQL = $strSQL . " WHERE billrate > 0 AND NOT VOID AND wt.ExportDate <> '0000-00-00 00:00:00' ";
// AND (wt.invoice_number IS NULL OR wt.invoice_number = 0) ";

if (!empty($_REQUEST['company_name'])) {
	$strSQL = $strSQL . " AND wt.company_name like '%". $_REQUEST['company_name'] . "%' ";
}
if (!empty($_POST['week_ending_start'])) {
	$strSQL = $strSQL . " AND wt.Weekending >= '". $_POST['week_ending_start'] . " 00:00:00' ";
}
if (!empty($_POST['week_ending_start'])) {
	$strSQL = $strSQL . " AND wt.Weekending <= '". $_POST['week_ending_end'] . " 00:00:00' ";
}
if ($_REQUEST['billing_cycle'] == "Monthly") {
	$strSQL = $strSQL . " AND wt.billing_cycle = 'Monthly' ";
}
if ($_REQUEST['billing_cycle'] == "Weekly") {
	$strSQL = $strSQL . " AND (wt.billing_cycle = 'Weekly' OR wt.billing_cycle IS NULL OR wt.billing_cycle = '') ";
}
 	$strSQL = $strSQL . " GROUP BY PO, wt.AssignmentNumber, wt.Employee_ID, wt.Hours, wt.WeekEnding) AS subquery";
//	$strSQL = $strSQL . " GROUP BY PO, wt.AssignmentNumber, wt.Employee_ID, wt.Hours, wt.WeekEnding, ANY_VALUE(wt.first_name), ANY_VALUE(wt.last_name), c.company_name, oj.organization) AS subquery";


$strSQL = $strSQL . " 
LEFT JOIN (
    SELECT 
        TRIM(oj.po_number) AS PO,
        SUM(IFNULL(oj.bill_rate, 0) * wt.Hours) AS POBILLED
    FROM 
        ic_timesheets wt
    LEFT JOIN 
...
$strSQL = $strSQL . " GROUP BY 
        PO
) oj ON oj.PO = subquery.PO
WHERE 
    (INVNUM IS NULL OR INVNUM = 0)"; 

$strSQL2 = $strSQL ." ORDER BY 
    subquery.COMPANY ASC, subquery.PO ASC, subquery.APEMAIL ASC, subquery.EMPID ASC, subquery.WKEND ASC;";
// . " ORDER BY COMPANY ASC, APEMAIL ASC, EMPID ASC, WKEND ASC";
...
while ($row = mysqli_fetch_array($result)) {
    ...
    echo '<tr align="center">';
    echo '<td>';
	if(!is_null($row['JOB']) && ($row["POAMT"] >= $row["POBILLED"] || $row["POAMT"] == 0 || empty($row["POAMT"]))) {
	...
	echo "<td>".  $row["PO"]."</td>";
	echo "<td align = 'right'>".  number_format($row["POAMT"],2)."</td>";
	if($row["POAMT"] >= $row["POBILLED"] || $row["POAMT"] == 0 || empty($row["POAMT"])) {
		echo "<td align = 'right'>".  number_format($row["POBILLED"],2)."</td>";
	} else {
		echo "<td align = 'right'><font color='red'>".  number_format($row["POBILLED"],2)."</font></td>";		
	}
    echo '</tr>';
	$invnum = $invnum + 1;
	$grand_total = $grand_total + ROUND($row['BILLRATE'] * $row['HOURS'],2);
}

// Close the table and add the submit button
echo '</table><br>';
echo '<b>Total Un-Invoiced Amount: '. number_format($grand_total,2). '</b><p>';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	for($x=0; $x<count($recipients); $x++) {
 		$query2 = $strSQL . " AND UNUMB = '". $recipients[$x]."'";
		// echo $query2;
		...
		If ($org_test !== $row3['JOB'] || $po_test !== $row3['PO']) { 
	...
	for($z=0; $z<count($recipients); $z++) {
		...
		// $contact_array = Contact_Info($link,$organization);
 		$query2 = $strSQL . " AND UNUMB= '". $recipients[$z]."'";
// 		AND wt.Assignment_ID = '". $PROJ[$z]."' AND wt.Weekending = '" .$WKEND[$z]. "'";
		...
		// $contact_array = Contact_Info($link,$organization);
		If ($org_test !== $row2['JOB'] || $po_test !== $row2['PO']) { 
			$invnum = $invnum + 1;
		}
		$org_test = $row2['JOB'];
		...
		$po_test = $row2['PO'];
			// write formula for duedate here
			$due_date =  date('m/d/Y', strtotime(' + '.$terms.' days'));
			$text = '"'.$invnum.'","'.
			$row2['COMPANY'].'","'.
			$contact_array['email'].'","'.
			date('m/d/Y').'","'.
