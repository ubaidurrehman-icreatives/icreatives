...
?>
<form method="post">
Start Invoice #:  <input type="number" id="startinvoice" name="startinvoice" value = 0 required >
...
if($_REQUEST['startinvoice'] > 0) {
	$sql = $sql . "WHERE invoice_number > 0 AND void IS FALSE AND invoice_number >= ".$_REQUEST['startinvoice']." ";
} else {
	$sql = $sql . "WHERE WHERE invoice_number > 0 AND void IS FLASE ";
}
if($_REQUEST['endinvoice'] > 0) {
	$sql = $sql . "AND invoice_number <= ".$_REQUEST['endinvoice']. " ";
}
