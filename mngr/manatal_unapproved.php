...
<h1>MANATAL UNAPPROVED TIMESHEET IMPORT</h1>
<?php
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && is_array($_POST['recipients'])) {
	...
	// make header
	 ...
	 $text = '"CO Code","Batch ID","File #","Employee Name","Temp Dept","Temp Rate","Reg hours","O/T hours","Other Period Beginning Date","Other Period Ending Date","Regular Earnings"'. PHP_EOL;
...
// 		AND wt.Assignment_ID = '". $PROJ[$i]."' AND wt.Weekending = '" .$WKEND[$i]. "'";
		...
		$update_sql = "UPDATE ic_timesheets SET ExportDate = NOW(),ApproveDate=NOW() WHERE  Unique_ID = '". $recipients[$i]."'";
		// echo $update_sql;
