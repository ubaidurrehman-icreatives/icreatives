...
//   $sScreen = $sScreen . "<td width=|260|  height=|170| align=|left| valign=|top|><a href=|https://www.icreatives.com| border=0><img src=|cid:logo.gif| width=|110| height=|123| border=|0| alt=|i creatives logo| style=|margin:15px 0px| /></a></td> ";
  ...
  $sScreen = $sScreen . "<tr> ";
	...
	if (!empty($row2["PoNumber"])) {
   		$sScreen = $sScreen . "<td align=|right|><b>PO Number:  " . $row2["PoNumber"] . "</b></td> ";
	}
    
    $sScreen = $sScreen . "</tr> ";
  $sScreen = $sScreen . "<tr> ";
    $sScreen = $sScreen . "<td height=|40px| align=|left| valign=|top|><b style=|color:#a4100c|>Customer: </b> " . str_replace("_"," ",addslashes($row2["JOB"])) . "</td> ";
...
for ($x = 1; $x <= 7; $x++) {
 $sScreen = $sScreen . "<tr>  "   ;
    $sScreen = $sScreen . "<td align=|left| ><b>". jddayofweek($x-2,1) . "</b></td>  "   ;
	   $sScreen = $sScreen . "<td align=|left|><span style=|color:#666|>". HrComp($row2["INHR" . $x], $row2["OUTHR" . $x])  . ":" . MinComp($row2["INHR" . $x])   . " " . AmPmCompIn($row2["INHR" . $x])    .  "</span></td>  " ;
    $sScreen = $sScreen . "<td align=|left|><span style=|color:#666|>". HrComp($row2["OUTHR" . $x],00) . ":" . MinComp($row2["OUTHR" . $x])  . " " . AmPmCompIn($row2["OUTHR" . $x])   .  "</span></td>  "  ;
    $sScreen = $sScreen . "<td align=|left|><span style=|color:#666|>". HrComp($row2["BREAK" . $x],00) . ":" . MinComp($row2["BREAK" . $x]) . "</span></td>  "  ;
  $sScreen = $sScreen . "<td align=|right|><b>" . ( $row2['OUTHR'. $x] - $row2['INHR' . $x] - $row2['BREAK' . $x] ) . "</b></td>  "   ;
  
 $sScreen = $sScreen . " </tr>  "   ;
$gtotal = $gtotal + ($row2['OUTHR'. $x] - $row2['INHR' . $x] - $row2['BREAK' . $x]); 
}
  ...
  $sScreen = $sScreen . "<tr> ";
	...
	if ($row2["CONT"] == "1") { 
	 ...
	 $sScreen = $sScreen . "</td> ";
    $sScreen = $sScreen . "<td colspan=|3| align=|right|><b style=|color:#a4100c|>Total: </b><b> " . $gtotal. "</b></td> ";
    $sScreen = $sScreen . "</tr> ";
...
// $_SERVER['HTTP_HOST']
    ...
    $sScreen = $sScreen . "</tr> ";
  $sScreen = $sScreen . "<tr> ";
    $sScreen = $sScreen . "<td colspan=|2| align=|left| valign=|bottom| height=|20px|><b>" . $row2["xORDER"] ."</b></td> ";
  $sScreen = $sScreen . "</tr> ";
                $sScreen = $sScreen . "</table> ";
  ...
  $sScreen = $sScreen . "<tr> ";
    $sScreen = $sScreen . "<td colspan=|2|> ";
      ...
      $sScreen = $sScreen . "<table width=|790| height=|100| border=|0| cellpadding=|0| cellspacing=|0| style=|border-spacing: 0; margin-left: auto; margin-right: auto; margin-top: 0; margin-bottom: 0;|> <tr> ";
          ...
          $sScreen = $sScreen . "<td width=|790| height=|100| align=|center| valign=|middle| style=| font-weight:bold ;font-size:11px; color:#FFF | background=|/webtime/email/images/footer/footer-repeat.gif| bgcolor=|#9C0000|  > ";
	  ...
	  $sScreen = $sScreen . "</td> ";
          $sScreen = $sScreen . "<td align=|right| valign=|top| > </td> ";
