...
echo "<h1>MANATAL EDIT INVOICES</h1>";
...
?>
<form method="get">
Start Date #:  <input type="date" id="startenter" name="startenter" value = "<?php echo $_REQUEST['startenter'] ;?>" >
End Date #:  <input type="date" id="endenter" name="endenter" value =  "<?php echo $_REQUEST['endenter'] ;?>" >
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["edit"])) {
        ...
        foreach ($_POST["edit"] as $Unique_id => $new_values) {
			...
			$AppoveDate = mysqli_real_escape_string($conn, $new_values["AppoveDate"]);
...
if($_REQUEST['startenter'] > '00-00-00') {
	// $sql = $sql . "WHERE void = FALSE AND  WeekEnding >= '".$_REQUEST['startenter']."' ";
	$sql = $sql . "WHERE WeekEnding >= '".$_REQUEST['startenter']."' ";
} else {
	// $sql = $sql . "WHERE void = FALSE ";
	$sql = $sql . "WHERE (void = FALSE OR void = TRUE) ";
}

if($_REQUEST['endenter'] > 0) {
	$sql = $sql . "AND WeekEnding <= '".$_REQUEST['endenter']. "' ";
}
echo $sql = $sql . " order by WeekEnding LIMIT 50";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
		...
		// echo $url = "https://www.icreatives.com/mngr/manatal_view_invoice.php?invnum=".encrypt_string($invoice_number);
// <a href="javascript:void(0);" onclick="openPopup('https://example.com');">Open Pop-up Window</a>

        // Create input fields for editing
        echo "<tr>"; 
		echo "<td><input type='text' style='width:70px;'  name='edit[$Unique_id][invoice_number]' value='$invoice_number'></td>";
		echo "<td style='width:100px;'>". date("Y-m-d",strtotime($row["WeekEnding"]))."</td>";
		...
		echo "<td><input type='date' name='edit[$ApproveDate][invoice_date]' value='$ApproveDate'></td>";
		...
		if($void <> 1){
			...
			echo "<option value = 0> </option>
					<option value = 1 selected >Void  </option>";
		}
