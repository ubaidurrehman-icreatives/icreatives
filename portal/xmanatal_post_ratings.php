...
if (isset($_SESSION['users_arr'])) {$users_arr = $_SESSION['users_arr'];}
...
*/
  ...
  foreach($mediums as $medium) {
    $seperator = ($medium != end($mediums)) ? "\n" : '';
    $query .= str_replace("'","''","\t".$medium_names[$medium].": ".$medium_info[$medium - 1].$seperator);
  }
  $query .= "', @EmployeeKey = '".addslashes($employeeID)."', @CustomerKey = '".addslashes($customerID)."', @DivisionKey = '".addslashes($divisionID)."', @ContactKey = '".addslashes($contactID)."', @OrderKey = '".addslashes($orderID)."', @AssignmentKey = '".addslashes($assignmentID)."'";
...
// add email to recruiter SJC 09/11/2020
    ...
    // Recipients
    $mail->setFrom('noreply@icreatives.com','icreatives');
