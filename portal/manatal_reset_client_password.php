...
if(!isset($_REQUEST['selector']) || !isset($_REQUEST['validator'])) { ?>
  ...
  <div class="container custom">
    <h1>Error</h1>
    <p>There was an error processing your request. Your password reset ticket may have expired.<p> Please contact your recruiter <!-- <a href="/portal-login">here</a> then click on reset on the following page--></p>
  </div>
  <?php
  return;
}
...
if ( ctype_xdigit( $selector ) && ctype_xdigit( $validator ) ) {
  ...
  // Get unexpired token with selector
	$query = "SELECT * FROM ic_password_reset_tickets WHERE selector = '". $selector. "' AND expires_at >= now() AND closed = 0";	
	...
	// see if they have logged in before, if so, skip the EULA
	$query = "SELECT * FROM ic_password_reset_tickets WHERE contact_id = '". $contact_id. "' AND closed = 1 AND close_reason = 'password created successfully'";	
  ...
  // Invalid selector
    ...
    <div class="container custom">
      <h1>Error</h1>
     <p>There was an error processing your request. Your password reset ticket may have expired.<p> You can request a new reset ticket by entering your email <a href="/portal-login">here</a> then click on reset on the following page</p>
    </div>
    <?php
    return;
  }
  ...
  <div class="container custom">
    <div class="row justify-content-center">
      <form action="/portal/manatal_client_process_create.php" class="form-login" id="form" method="post">
        <h1 class="form-login-heading"><span style="color: #b22625;">Reset Password</span></h1>
        ...
        <div class="form-group text-right">
		  ...
		  <p><a href="/portal/manatal_client_portal_login.php">Cancel</a></p>
        </div>
        <div id="message">
          <?php
  ...
  document.getElementById('form').addEventListener('submit', function(e) {
   ...
   <?php } else { ?>
    ...
    <div class="container custom">
        <div class="row justify-content-center">
            <form action="/portal/manatal_client_process_create.php" class="form-login" id="form" method="post">
                <h1 class="form-login-heading" style="color: #b22625;">Create Password</h1>
                <p>Please enter a password 6 characters or more.</p>
                <p>Your password must contain a combination of<br>UPPER CASE, lower case, and either $pecial or num3ric characters.</p>
                <input type="hidden" name="selector" value="<?php echo $selector; ?>">
                <input type="hidden" name="validator" value="<?php echo $validator; ?>">
