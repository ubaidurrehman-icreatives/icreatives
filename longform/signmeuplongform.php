...
$candidate_id = $_GET['CID']; // CID = Candidate ID

$recruiter_id = $_GET['uid'] ?? '';
...
try {
    ...
    echo "<p> </p><p>The server is busy or you have a link. Please wait {$wait} seconds and try again.</p>";
...
$_SESSION['candidate_arr'] = $candidate_arr;

$formpath = "/LongForm";
$_SESSION['formpath'] = $formpath;
$_SESSION['signupform'] = 1;



if (empty($recruiter_id)) {

if (!empty($candidate_arr['owner'])) {
	$recruiter_id = $candidate_arr['owner']; 
} else {
	$recruiter_id = $candidate_arr['creator']; 
}
}

// assign worktype radio buttons from json
// Always initialize to safe defaults
$worktype_full_time = '';
$worktype_contract = '';
$worktype_contract_to_hire = '';
$worktype_part_time = '';

// Normalize and assign if data exists
if (!empty($candidate_arr['custom_fields']['worktype']) && is_array($candidate_arr['custom_fields']['worktype'])) {
    $normalized = array_map('trim', array_map('strtolower', $candidate_arr['custom_fields']['worktype']));

    // Assign "checked" based on inclusion
    $worktype_full_time        = in_array('full-time', $normalized)        ? 'checked' : '';
    $worktype_contract         = in_array('contract', $normalized)         ? 'checked' : '';
    $worktype_contract_to_hire = in_array('contract to hire', $normalized) ? 'checked' : '';
    $worktype_part_time        = in_array('part time', $normalized)        ? 'checked' : '';
}

// Find recruiter email address and name

// $client = new \GuzzleHttp\Client();

$response = $client->request('GET', 'https://api.manatal.com/open/v3/users/'.$recruiter_id.'/', [
  'headers' => [
    'Authorization' => $token,
...
$response->getBody();
$responseStr = $response->getBody();
$recruiter_arr = json_decode($responseStr, true);
$recruiter_email = $recruiter_arr['email'];
$recruiter_name = $recruiter_arr['full_name'];


	// $link = mysqli_connect('localhost', 're0nm8', '50h8r6WNvB!ozVY2','ck3b2t') or die("Error: " . mysqli_error());
        // global $signupform;
         require_once($_SERVER['DOCUMENT_ROOT'] . '/custom-php/ic_functions.php');
         require_once($_SERVER['DOCUMENT_ROOT'] . '/custom-php/list.php');
         // require_once($_SERVER['DOCUMENT_ROOT'] . '/custom-php/signup_process.php');
		echo '<html><meta name="viewport" content="width=device-width, initial-scale=1.0"><body>';
        include ($_SERVER['DOCUMENT_ROOT'] . '/longform/javascript.php');
		echo "</body></html>";
        $portal="TALENT";
...
<?php
function isMobile() {
    return preg_match('/(android|iphone|ipad|ipod|blackberry|webos|windows phone|opera mini|iemobile)/i', $_SERVER['HTTP_USER_AGENT']);
}
	if (isMobile() ) { ?>
   		<div style='clear:both; float:left: width:650px;'>
			...
			<script>
				...
				if(mql.matches) {  
					...
					alert("PLEASE TURN TO YOUR PHONE HORIZONTAL");
				} else {  
...
<!-- Empty container to inject the form into -->
<?php
  include($_SERVER['DOCUMENT_ROOT'] . '/longform/templates/signmeuplongform.tpl.php');
?>
