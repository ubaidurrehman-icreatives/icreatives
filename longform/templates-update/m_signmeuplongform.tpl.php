<?php
$EID = substr($_GET["EID"],1,8);
include $_SERVER['DOCUMENT_ROOT']."/wp-content/themes/vg-mirinae/db5.php";
$strSQL = "select UserDefined1 from CFG_BRANCHPROFILE where UserDefined1 is not null";

$resSkills = odbc_exec($conn,$strSQL); 

while($row = odbc_fetch_array($resSkills)){ 

 echo $row['UserDefined1'];
}
...
<?php
include $_SERVER['DOCUMENT_ROOT']."/wp-content/themes/vg-mirinae/db5.php";
$sql = "SELECT Code, Description FROM IO_RecruitingSource order by Code ASC" ;
$result = odbc_exec($conn,$sql) or die("Couldn't execut query");
