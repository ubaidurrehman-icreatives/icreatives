...
$link = mysqli_connect('localhost', 'TempBack', 'XE5Vx@54Pu1IRQXa','tempback') 
    or die("Error: " . mysqli_error($link));

// Prepared statement
$stmt = $link->prepare("
    SELECT tb_user.api_token, tb_user.company, tb_companies.id as subscriber_id, tb_tokens.id 
    FROM tb_user 
    JOIN tb_companies ON tb_user.company = tb_companies.id 
	JOIN tb_tokens ON tb_tokens.manatal_company_id = tb_companies.manatal_company_id 
    WHERE tb_companies.manatal_company_id = ?
");
if (!$stmt) {
    die("Prepare failed: " . $link->error);
}

// Bind parameter (s = string, change to "i" if it's integer)
$stmt->bind_param("s", $manatalId);
...
if ($result->num_rows === 0) {
    echo "<div style='padding-top:25%; color:red;'><h1><center>Alert:<p>URL has an incorrect or missing company ID.<p>Please call your recruiter.</center></h1></div>";
    $apiToken = null;
	exit();
} else {
    ...
    $access_token = $row['id'];
    $manatal_id = $row['company'];
    $subscriber_id = $row['subscriber_id'];
    // echo "API Token: " . htmlspecialchars($apiToken);
...
$_SESSION['manatal_id'] = $manatal_id;
...
if (mysqli_num_rows($result) > 0) {
	// uncommment below when portal is activated
	// header("Location: /client-portal-login/?user=$user");
	header("Location:/portal/?r=recognize".($order != "" ? "&orderID=$order" : ""));
	exit;
}
...
//	and icreativesportalaccess";
	...
	$query = "select * from ic_matches where candidate_email = '". $user . "' AND closed = 0";
	$result = mysqli_query($link,$query);
	...
	if(!$is_contact && !$is_resource) {
		...
		header("Location: /login.php/?&r=recognize");
	exit;}
	...
	header("Location:/choose-portal/?o=$order&user=$user&company_id=$company_id");
      ...
      header("Location:/portal/customer/manatal_talent_portal_signin.php/?user=".$user);
      exit;
    }
	  else if ( $is_contact){
