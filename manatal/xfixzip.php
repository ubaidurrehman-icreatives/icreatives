...
try {
    ...
    // ------------- Composer autoload (open_basedir-safe) -------------
...
require_once  dirname(__DIR__) . '/db/token.php';
    ...
    if (is_object($loader) && method_exists($loader, 'add')) { // PSR-0 (older)
    ...
    if (!$applicantId) {
        if ($DEBUG) { echo "No id in payload—ignored."; }
        ...
        while (!empty($words)) {
            ...
            try {
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    // debug only
                    if (isset($row['city'], $row['stateISO']) && !headers_sent()) {
                        // avoid echo in webhook unless debug
                    }
                    return $row['zipCode'];
                }
    ...
    // If no ZIP from resume text, try DB by city/state
    if (!$zipCode) {
    ...
    // Normalize ZIP (pad leading zeros for US 5-digit zips)
    if ($zipCode) {
        $zipDigits = preg_replace('/\D+/', '', $zipCode);
        if ($zipDigits !== '' && strlen($zipDigits) < 5) {
            $zipDigits = str_pad($zipDigits, 5, '0', STR_PAD_LEFT);
        }
        $zipCode = $zipDigits;
    }
    ...
    $needsPatch =
        (!isset($manatalid) || $manatalid !== $applicantId) ||
        (!isset($oldZip)    || $zipCode !== $oldZip);
    ...
    if ($needsPatch) {
        ...
        if ($DEBUG) { echo $zipCode; }
    } else {
        if ($DEBUG && (!$zipCode || $zipCode === '')) {
    ...
    // If not debugging, return a clean empty success for webhook
    if (!$DEBUG) {
        http_response_code(204); // success, no content
    } else {
        // keep default 200 with debug output
    }
    ...
    if ($DEBUG) {
        echo "\nInternal error: " . htmlspecialchars($e->getMessage(), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "\n";
    }
} finally {
