...
$row = odbc_fetch_array($resMySel);
	...
	$InvDate = $row["INVDATE"];
	$TERMS = $row["TERMS"];
	$DUEDATE = date("m/d/Y", strtotime($row["DUEDATE"])+ (intval($row["TERMS"]) * 60 * 60 *24) );
...
IF ( !empty($row["xORDER"]) ) {
	...
	while($row = odbc_fetch_array($resMySel)){
		...
		<div style=" float:left; text-align:right; width:700px;">
			...
			<div style="clear:right; float:right; padding:0px 2px 0 0; width:230px;">
				<span style="color:#000000; margin:0; font-size:20px">
	   			<b>inv no:</b>
	   			<?php echo( SubStr($row["INVNUM"], -6) ); ?></span>
      			</div>
      			<div style="clear:right; float:right; padding:0px 2px 0 0; width:230px;">
				<span style="color:#000000; margin:0; font-size:20px">
				<b>page no:</b>
			...
			<div style="clear:right; float:right; padding:0px 2px 0 0; width:230px;">
				<span style="color:#000000; margin:0; font-size:20px">
				<b>date:</b>
				<?php echo( Date("m/d/Y", StrToTime($row["INVDATE"]) )); ?></span>
      			</div>
			<div style="clear:right; float:right; padding:0px 2px 0 0; width:300px;">
				<span style="color:#000000; margin:0; font-size:20px">
					<?php echo "<b>customer no:</b>" . $row["CUSTOMERID"]; ?>
			</div>
			<div style="clear:right; float:right; padding:0px 2px 0 0; width:300px;">
				<?php if (!empty($row["PO"])) {?>
					<span style="color:#000000; margin:0; font-size:20px">
					<b>po no:</b>
		 			<?php echo( $row["PO"]); ?></span>
				<?php } ?>
      			</div>
      			<div style="clear:right; float:right; padding:0px 2px 0 0;  width:230px;">
				<?php if ($row["VENDORNO"] <> "" ) {?>
		...
		<!--- End New for walmart 07/2015 --->
			...
			<div style="clear:left; float:left; padding: 10px 0 0 0px; font-size:14px;  width:720px;">
				...
				<?php } ?>
				<?php echo( $row["BPCITY"] . ", " . $row["BPSTATE"] . " " . $row["BPZIP"] ) ;?>
	 		</div>
		...
		<?php } // extra bracket should not be ?><?php }  ?>
		...
		<div style="float:left;  width:280px">
			...
			$DateDiff = round ( (time() - strtotime($InvDate))  / (60 * 60 * 24));
